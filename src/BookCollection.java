...
public class BookCollection {
	...
	// stream version of findBooks
	public List<Book> findBooksUsingStream(Predicate<Book> searchCriteria) {
		return books.stream().filter(searchCriteria).collect(Collectors.toList());
	}

	// stream version that return matched titles as a Map<id, title>
	public Map<Integer, String> findTitles(Predicate<Book> searchCriteria) {
		// return books.stream().filter(searchCriteria).collect(Collectors.toMap((Book b) -> b.getId(), (Book b) -> b.getTitle()));

		// more compact
		return books.stream().filter(searchCriteria).collect(Collectors.toMap(Book::getId, Book::getTitle));
	}

	// stream version that return number of matches
	public int findNUmberOfBooks(Predicate<Book> searchCriteria) {
		return (int) books.stream().filter(searchCriteria).count();
	}

	// <--- end Added in chapter 17
